...
RSpec.shared_examples "an OrgaAdmin with application Ability" do
...
RSpec.describe "User", :type => :model do
  ...
  describe "with ability assigned to user" do
	...
	authorizable: user, 
	role: wobauth_roles(:orga_admin))
    }
    context "on Ability" do
      subject(:ability) { Ability.new(user) }
      it_behaves_like "an OrgaAdmin with application Ability"
    end

    context "on Wobauth::AdminAbility" do
      subject(:ability) { Wobauth::AdminAbility.new(user) }
      it_behaves_like "an OrgaAdmin with Wobauth::AdminAbility"
    end
  end
